    ...
    {:ok, _} ->
      ...
      case GreenManTavern.MindsDB.SQLClient.query(test_query) do
        {:ok, %{"data" => data}} when is_list(data) and length(data) > 0 ->
          IO.puts("   🎉 #{model_name} WORKING! Response: #{inspect(Enum.at(Enum.at(data, 0), 0))}")
        {:ok, result} ->
...
system_queries = [
  ...
  {"Information schema", "SELECT table_name FROM information_schema.tables WHERE table_schema = 'mindsdb';"}
]
