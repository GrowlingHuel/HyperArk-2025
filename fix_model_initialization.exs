...
agents = ["student_agent", "grandmother_agent", "farmer_agent", "robot_agent", "alchemist_agent", "survivalist_agent", "hobo_agent"]
  ...
  case GreenManTavern.MindsDB.SQLClient.query(create_agent) do
    {:ok, _} ->
      ...
      case ModelWaiter.wait_for_model(agent_name) do
        :ready ->
          # Test the model
          test_query = "SELECT answer FROM mindsdb.#{agent_name} WHERE question = 'Hello, how are you?';"
...
final_test = "SELECT answer FROM mindsdb.#{test_agent} WHERE question = 'What is permaculture and why is it important?';"
