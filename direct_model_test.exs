  ...
  defp test_loop(model_name, question, attempt, max_attempts) do
    ...
    case GreenManTavern.MindsDB.SQLClient.query(query) do
      ...
      {:ok, %{"type" => "error", "error_message" => error_msg}} ->
        if String.contains?(error_msg, "not found") or String.contains?(error_msg, "does not exist") do
      ...
      {:error, reason} ->
        IO.puts("   ⏳ #{model_name} connection issue (attempt #{attempt}/#{max_attempts}): #{inspect(reason)}")
        :timer.sleep(3000)
        test_loop(model_name, question, attempt + 1, max_attempts)
    end
  end
...
final_test_query = "SELECT answer FROM mindsdb.student_agent WHERE question = 'What is permaculture?';"
